...
(mca$var$coord[1:I,1:ncp]%*%diag(1/sqrt(mca$eig[1:ncp,1])))/(ca$row$coord[,1:ncp]%*%diag(1/sqrt(ca$eig[1:ncp,1])))
(mca$var$coord[(1+I):(I+J),1:ncp]%*%diag(1/sqrt(mca$eig[1:ncp,1])))/(ca$col$coord[,1:ncp]%*%diag(1/sqrt(ca$eig[1:ncp,1])))

# Attention pour les valeurs propres :
aux = CA(t(tab.disjonctif(don))%*%tab.disjonctif(don))
mca$eig[1:3,1]^2 / aux$eig[1:3,1]
```
